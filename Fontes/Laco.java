...
class Laco{
	...
	public int ExecutaFor(int TOPO){
		...
		String ExpFor = "";
		ExpFor = Inter.linhas[TOPO].replaceAll("VOOR", "");//Retira a palavra VOOR da linha;
		ExpFor = ExpFor.replaceAll("[\\{\\} ]", ""); // Remove os espaços ;
		String[] PartesFor = ExpFor.split("\\?");//Separa a linha em 3 partes apartir do ?. Exemplo: VOOR{ A<)0 ? A<<2 ? A...? }[ = PartesFor[0] = A<)0, PartesFor[1] = A<<2, PartesFor[2] = A...
		...
		if(!Inter.Operacao.ExpressoesComparacao(PartesFor[1])){//Caso a expressão der falsa entra nesse if;
			while(!Inter.linhas[posicao].startsWith("]V")){//Ignora todas linhas até a chave que fecha o VOOR;
				posicao = IgnoraOutrosFors(posicao);//Ignora os outros LOOPINGS que estiver dentro do VOOR falso;
				posicao++;
			}
		} else {
			do{
				posicao = TOPO+1;//Recebe a linha a baixo do VOOR;
				while(!Inter.linhas[posicao].startsWith("]V")){//Caso encontrar a chave do VOOR termina a execução;				
		...
		TOPO = posicao;//Retorna a chave do VOOR;
		return TOPO;
	}
	...
	public int ExecutaWHILE(int TOPO){
		...
		ExprecaoWhile = ExprecaoWhile.replaceAll("GIRAGIRA\\{", ""); // So copia a expreção por Exemplo : Antes-> GIRAGIRA{a<<b}[    Depois-> {a<<b}[;
		posicao++; // Avança para linha de baixo do GIRAGIRA;
